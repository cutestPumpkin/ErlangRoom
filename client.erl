...
loop() ->
  receive
    {tcp, _Socket, Bin} ->
      io:format("~ts ~ts ~n", ["收到消息：", binary_to_term(Bin)]),
      loop();
    {tcp_closed, _Socket} ->
      io:format("~ts ~n", ["连接关闭！"])
  end.
...
%%  S = io:get_line(list_to_binary("选择你的操作(1注册，2登录，3注销，4私聊，5群聊):")),
  S = io:get_line("选择你的操作(1注册，2登录，3注销，4私聊，5群聊):"),
...
%% 用户注册
operation_message(1) ->
  I = io:get_line("输入注册用户id（整数）："),
  {Id, _Info} = string:to_integer(I),
  Password = io:get_line("输入注册密码："),
  [Id, register_user, Password, 0, 0];
%% 用户登录
operation_message(2) ->
  I = io:get_line("输入你的用户id（整数）："),
  Password = io:get_line("输入你的登录密码："),
  {Id, _Info} = string:to_integer(I),
  [Id, login_user, Password, 0, 0];
%% 用户退出
operation_message(3) ->
  I = io:get_line("输入你的用户名id（整数）："),
  {Id, _Info} = string:to_integer(I),
  [Id, login_out, 0, 0, 0];
%% 私聊
operation_message(4) ->
  Sd = io:get_line("输入发送信息的他人用户id（整数）："),
  Msg = io:get_line("发送信息："),
